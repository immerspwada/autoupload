  ...
  return `
    <div class="seo-section">
      <div class="page-header">
        <h2>SEO Auto-Optimization</h2>
        <p>ตั้งค่าการสร้าง title, description, tags, category อัตโนมัติ เพื่อเพิ่มยอดดูและรายได้จากโฆษณา YouTube</p>
      </div>
      ...
      <div class="card" style="margin-bottom:16px">
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <select id="seo-mode">
              <option value="auto">อัตโนมัติ — สร้าง metadata ให้ทุกครั้ง</option>
          ...
          <div class="form-group">
            <label for="seo-title-template">Title Template <span style="font-weight:400;color:var(--text-muted)">(ไม่บังคับ)</span></label>
          ...
          <div class="form-group">
            <label for="seo-preferred-hour">ชั่วโมงที่ต้องการ <span style="font-weight:400;color:var(--text-muted)">(0–23, ไม่บังคับ)</span></label>
            <input type="number" id="seo-preferred-hour" min="0" max="23" placeholder="เช่น 19" style="max-width:120px">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <p class="section-desc" style="margin-bottom:16px">ลองใส่คำอธิบายวิดีโอ TikTok เพื่อดูตัวอย่าง title/tags/category ที่ระบบจะสร้างให้</p>
          <div class="form-group">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div class="form-group">
              <label for="seo-preview-author">Author <span style="font-weight:400;color:var(--text-muted)">(ไม่บังคับ)</span></label>
              <input type="text" id="seo-preview-author" placeholder="tiktok_username">
            </div>
            <div class="form-group">
              <label for="seo-preview-duration">ความยาว (วินาที) <span style="font-weight:400;color:var(--text-muted)">(ไม่บังคับ)</span></label>
              <input type="number" id="seo-preview-duration" placeholder="เช่น 45">
            </div>
          </div>
          <button id="btn-seo-preview" type="button" class="btn btn-secondary">ดู SEO Preview</button>
...
async function preview() {
  const btn = document.getElementById('btn-seo-preview');
  ...
  if (!desc) { window.app.showToast('กรุณาใส่คำอธิบายวิดีโอ', 'error'); return; }
  ...
  try {
    const res = await fetch('/api/seo/preview', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ desc, author, duration, schedulePublish: document.getElementById('seo-auto-schedule').checked })
    });
    ...
    btn.disabled = false; btn.textContent = 'ดู SEO Preview';
  }
}
...
function renderPreview(metadata, categoryName) {
  ...
  const qualityClass = quality.grade === 'excellent' ? 'success'
    : quality.grade === 'good' ? 'pending' : 'pending';
  ...
  container.innerHTML = `
    <div class="seo-preview-card">
      ...
      <div class="seo-preview-field">
        ...
        <div class="seo-score-row">
          ...
          <div>
            ...
            <small class="section-desc">Grade: ${window.app.escapeHtml(quality.grade)}</small>
          </div>
        </div>
      </div>
      ...
      <div class="seo-preview-meta">
        <span>${window.app.escapeHtml(categoryName)} (ID: ${metadata.categoryId})</span>
      ...
      ${metadata.validation.issues.length > 0 ? `
        <div class="seo-preview-issues">
          ${metadata.validation.issues.map(i => `
            <div class="seo-issue ${i.level}">
              ${i.level === 'error' ? '✕' : i.level === 'warning' ? '!' : 'i'}
              ${window.app.escapeHtml(i.message)}
            </div>`).join('')}
        </div>` : ''}
      ...
      ${quality.checks?.length ? `
        <div class="seo-preview-issues">
          ${quality.checks.map(c => `
            <div class="seo-issue ${c.level}">
              ${c.level === 'error' ? '✕' : c.level === 'warning' ? '!' : c.level === 'success' ? '✓' : 'i'}
              ${window.app.escapeHtml(c.message)}
            </div>`).join('')}
        </div>` : ''}
    </div>`;
}
